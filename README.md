...
flowchart TD
    ...
    classDef build fill:#c6dcff,stroke:#305bab
    classDef checks fill:#ffc6c6,stroke:#bd0909
    classDef shipped fill:#fff6b6,stroke:#af7e02
    classDef runtime fill:#dedaff,stroke:#6631d7

    skills[6 Miro skills<br/>browse · diagram · doc · table<br/>code-review · code-spec]
    power[Kiro power<br/>code-gen: design to code]
    docs[docs/<br/>20 guides: setup · platforms · MCP tools]

    val[validation/<br/>8 JSON schemas · frontmatter · shell checks]
    conv[converters<br/>one source, five formats]
    claude[Claude Code plugin<br/>+ marketplace manifest]

    hook[Husky pre-commit<br/>runs bun run validate]
    gen[Generated copies<br/>Codex · Cursor · Cowork<br/>Gemini · Agent Skills]

    ci[GitHub Actions validate.yml<br/>every PR · packages Cowork zip on main]
    install[bun run plugins:install<br/>local or main, for testing]

    tools[AI coding tools<br/>Claude Code · Cursor · Codex · Gemini CLI · Kiro]
    mcp[Miro MCP server<br/>mcp.miro.com · OAuth 2.1]
    boards([Your Miro boards<br/>diagrams · docs · tables])
    ...
    skills --> claude
    conv --> gen
    val -->|before commit| hook
    hook -->|then on push| ci
    claude --> install
    gen --> install
    docs -->|install guides| tools
    claude -->|marketplace install| tools
    install --> tools
    power --> tools
    tools --> mcp
    mcp --> boards

    class skills,power authored
    class docs docsbox
    class val,hook,ci checks
    class conv,install build
